...
class BenefitsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        yield {
            'kind': 'benefit',
            'id': safe(f"benefit_{response.meta['domain']}", ''),
            ...
            'content': safe(page_content[:1000] + "..." if len(page_content) > 1000 else page_content, ''),
            'emails': safe(emails[:5], []),
            'phones': safe(phones[:5], []),
            'addresses': safe(addresses[:3], []),
            'social_media': safe(social_media[:5], []),
            'tags': safe(keywords[:10], [])
        }
