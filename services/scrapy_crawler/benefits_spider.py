...
class BenefitsSpider(scrapy.Spider):
    ...
    custom_settings = {
        'FEED_FORMAT': 'json',
        'FEED_URI': '../../data/benefits/entries.json',
        'LOG_LEVEL': 'INFO',
    }

    def start_requests(self):
        with open('../../data/sources/benefits.json', encoding='utf-8') as f:
            config = json.load(f)
        for domain in config:
            seed_urls = domain.get('seed_urls', [])
            for url in seed_urls:
                yield scrapy.Request(url, meta={'domain': domain.get('domain'), 'keywords': domain.get('keywords', [])})
