...
function getSummary(entry: Entry) {
  ...
  return entry.summary_de || 'Keine Zusammenfassung verfuegbar.';
}
...
export default function ResultCard({ result }: { result: Entry }) {
  ...
  const iqs = result.iqs != null ? Number(result.iqs) : result.qualityScores?.iqs != null ? Number(result.qualityScores.iqs) : null;
  const ais = result.ais != null ? Number(result.ais) : result.qualityScores?.ais != null ? Number(result.qualityScores.ais) : null;
  ...
  return (
    <Link to={`/entry/${result.id}`} className="block">
      <Card className="h-full border-border/70 transition hover:border-foreground/20 hover:shadow-sm">
        <CardContent className="space-y-4 p-4">
          ...
          <div className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
            ...
            <div>
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground">Data Quality</div>
