...
export default function ResultCard({ result }: ResultCardProps) {
  ...
  const sourceTier = result.provenance?.sourceTier;
  const institutionType = result.provenance?.institutionType;
  const jurisdiction = result.provenance?.jurisdiction;
  return (
    <Card className="mb-4">
      <CardContent className="p-4">
        ...
        {(sourceTier || institutionType || jurisdiction) && (
          <div className="mb-2 flex flex-wrap gap-2">
            {sourceTier && <Badge variant="outline">{sourceTier}</Badge>}
            {institutionType && <Badge variant="outline">{institutionType}</Badge>}
