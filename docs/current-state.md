...
- Trusted topic profiles can now also sync curated domain seeds via `python crawlers/cli.py sync-topic-seeds --domain <domain>`, which keeps `benefits` and `contacts` manifests aligned with the topic registry
- Trusted topic profiles now distinguish official information, trusted practical tools, and contextual discovery sources; synced `tools` seeds stay tool-focused while `aid` can include trusted glossary/context material such as Caritas glossaries or NGO studies
